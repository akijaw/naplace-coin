...
- **학생/플레이어**: 잔액·QR·거래내역 확인
- **부스**(몬티홀 게임 포함): 학번으로 지급/결제, API 연동
- **관리자**: **코인 지급(발행)**, **자동 결제**, 사용자·부스·API 키 관리
...
| `/` | 랜딩 (시스템 소개) |
| `/login` | 학생·관리자 로그인 |
| `/wallet` (`/me`) | 내 잔액·QR·거래내역 |
| `/ranking` | 개인·부스 랭킹 |
| `/club` | 부스 결제 콘솔 (QR 스캔 / 학번 자동 결제 / 활동 관리) |
| `/docs` | API 문서 |
| `/admin` | 관리자 콘솔 (코인 지급 · 자동 결제 · 사용자 · 부스/키 · 거래내역) |
...
async function buyChips(studentId, amount) {
  const result = await coin.createPaymentRequest(studentId, amount, "몬티홀 칩 구매");
